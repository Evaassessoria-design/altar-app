...
/** Usuários por transação na varredura diária. */
const USUARIOS_POR_LOTE = 100;

/**
 * Alertas de UM usuário. Extraído para que a varredura possa ser paginada sem
 * duplicar regra nenhuma.
 */
async function alertarUsuario(
  ctx: MutationCtx,
  user: Doc<"users">,
  now: Date,
): Promise<void> {
...
/**
 * Varredura diária de alertas — TODOS os assinantes, em lotes.
 *
 * ── POR QUE PAGINA ──────────────────────────────────────────────────────────
 * Antes eram `take(500)`: a partir do quingentésimo primeiro cadastro, as
 * contas seguintes simplesmente paravam de receber alerta de evento próximo,
 * de compra pendente e de trial vencendo — sem erro, sem aviso, sem nada. O
 * defeito só apareceria como "o ALTAR não me avisou", meses depois, e o teto
 * não estava documentado em lugar nenhum.
 *
 * Cada lote é uma transação curta que agenda a próxima. `agora` viaja junto de
 * propósito: uma varredura que começa às 23h59 e atravessa a meia-noite tem de
 * usar o MESMO dia do começo ao fim, senão a regra "só um aviso de trial por
 * dia" dispararia duas vezes na virada.
 */
export const generateDailyAlerts = internalMutation({
  args: {
    cursor: v.optional(v.union(v.string(), v.null())),
    agora: v.optional(v.number()),
  },
  handler: async (ctx, args) => {
    const now = new Date(args.agora ?? Date.now());

    const pagina = await ctx.db
      .query("users")
      .paginate({ numItems: USUARIOS_POR_LOTE, cursor: args.cursor ?? null });

    for (const user of pagina.page) {
      await alertarUsuario(ctx, user, now);
    }

    if (!pagina.isDone) {
      await ctx.scheduler.runAfter(0, internal.notifications.generateDailyAlerts, {
        cursor: pagina.continueCursor,
        agora: now.getTime(),
      });
    }

    return { processados: pagina.page.length, concluido: pagina.isDone };
  },
});

...
// ═════════════════════════════════════════════════════════════════════════════
// A DESPESA LEMBRA DE ONDE VEIO
//
// Manter o custo de uma compra desfeita PRECISA soltar o vínculo operacional —
// enquanto ele existe, a compra cancelada é uma inconsistência que cala a
// margem do evento. Mas o vínculo era a única coisa que ligava a despesa
// àquela compra, e soltá-lo deixava no livro uma linha órfã.
//
// Proveniência ≠ vínculo. A primeira é memória e não dirige nada; o segundo
// dirige tudo e tem de sumir. Mesmo papel de `assemblyItems.compositionId`.
// ═════════════════════════════════════════════════════════════════════════════
describe("proveniência da despesa quando a compra sai de cena", () => {
  it("cancelar mantendo o custo grava de qual compra ele nasceu", async () => {
    const { t, dona, compra, lancamentoDe } = await cenario();
    const id = await compra();
    await dona.mutation(api.purchases.registerCost, { id });
    const tx = (await lancamentoDe(id))!;
    ...
    const sobrou = (await t.run((ctx: MutationCtx) => ctx.db.get(tx._id)))!;
    expect(sobrou.origemCompra, "a despesa esqueceu de onde veio").toMatchObject({
      nome: "Rosas brancas",
      desfecho: "cancelada",
    });
    expect(sobrou.origemCompra!.em).toMatch(/^\d{4}-\d{2}-\d{2}$/);
  });

  it("e o vínculo operacional some do mesmo jeito — proveniência não o recria", async () => {
    // A trava desta rodada: guardar a origem não pode ressuscitar o vínculo,
    // senão a compra cancelada volta a ser `canceladaComLancamento` e a margem
    // do evento se cala para sempre.
    const { t, dona, compra, lancamentoDe, linha } = await cenario();
    const id = await compra();
    await dona.mutation(api.purchases.registerCost, { id });
    const tx = (await lancamentoDe(id))!;
    ...
    expect((await linha(id))!.transactionId, "o vínculo voltou").toBeUndefined();
    const vinculadas = await t.run((ctx: MutationCtx) =>
      ctx.db
        .query("purchaseItems")
        .withIndex("by_transaction", (q) => q.eq("transactionId", tx._id))
        .collect(),
    );
    expect(vinculadas, "a despesa continua sendo apontada por uma compra").toHaveLength(0);
  });

  it("excluir mantendo o custo sobrevive à compra — o nome fica no snapshot", async () => {
    // O caso em que um ponteiro não serviria: a linha da compra não existe
    // mais quando alguém for ler a origem.
    const { t, dona, compra, lancamentoDe, linha } = await cenario();
    const id = await compra();
    await dona.mutation(api.purchases.registerCost, { id });
    const tx = (await lancamentoDe(id))!;
    ...
    expect(await linha(id)).toBeNull();
    expect((await t.run((ctx: MutationCtx) => ctx.db.get(tx._id)))!.origemCompra).toMatchObject({
      nome: "Rosas brancas",
      desfecho: "excluida",
    });
  });

  it("o nome gravado é o do momento, não o do cadastro original", async () => {
    const { t, dona, compra, lancamentoDe } = await cenario();
    const id = await compra();
    await dona.mutation(api.purchases.registerCost, { id });
    const tx = (await lancamentoDe(id))!;
    await dona.mutation(api.purchases.updatePurchase, { id, name: "Rosas importadas" });

    await dona.mutation(api.purchases.setPurchaseStatus, {
      id, status: "cancelado", despesa: "manter",
    });

    expect((await t.run((ctx: MutationCtx) => ctx.db.get(tx._id)))!.origemCompra!.nome).toBe(
      "Rosas importadas",
    );
  });

  it("REMOVER não grava proveniência nenhuma — não sobra despesa para lembrar", async () => {
    const { t, dona, compra, lancamentoDe } = await cenario();
    const id = await compra();
    await dona.mutation(api.purchases.registerCost, { id });
    const tx = (await lancamentoDe(id))!;
  ...
  it("compra que nunca virou custo não inventa origem em lugar nenhum", async () => {
    const { t, dona, compra } = await cenario();
    const id = await compra();
    await dona.mutation(api.purchases.setPurchaseStatus, { id, status: "cancelado" });
    const todas = await t.run((ctx: MutationCtx) => ctx.db.query("transactions").collect());
  ...
  it("despesa viva e vinculada NÃO ganha origem — quem responde é o vínculo", async () => {
    // A origem só é gravada quando o vínculo é desfeito. Gravá-la antes criaria
    // duas respostas para a mesma pergunta, livres para divergir.
    const { dona, compra, lancamentoDe } = await cenario();
    const id = await compra();
    await dona.mutation(api.purchases.registerCost, { id });
    await dona.mutation(api.purchases.setPurchaseStatus, { id, status: "recebido" });
    ...
    expect((await lancamentoDe(id))!.origemCompra).toBeUndefined();
  });

  it("e nunca escreve na despesa de outra conta", async () => {
    // Ponteiro cruzado não deveria existir; se existir, a gravação da origem
    // passa pela MESMA conferência de dono que o resto de `aplicarDecisao`.
    const { t, dona, rival, compra, ids } = await cenario();
    ...
    await dona.mutation(api.purchases.setPurchaseStatus, {
      id: minha, status: "cancelado", despesa: "manter",
    });

    const alheio = (await t.run((ctx: MutationCtx) => ctx.db.get(txDaRival)))!;
    expect(alheio.origemCompra, "carimbou origem no lançamento de outra conta").toBeUndefined();
  });
});

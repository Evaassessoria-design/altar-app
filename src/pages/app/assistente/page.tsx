...
type Tarefa = Doc<"assistantTasks">;
...
export default function EscritorioPage() {
  const equipe = useQuery(api.assistente.equipe, {});
  const historico = useQuery(api.assistente.listar, {});
  const delegar = useMutation(api.assistente.delegar);
  const executar = useAction(api.assistenteExecutor.executar);

  const [pedido, setPedido] = useState("");
  const [agenteId, setAgenteId] = useState<string>("");
  const [enviando, setEnviando] = useState(false);
  const [aberto, setAberto] = useState<Id<"assistantTasks"> | null>(null);
  ...
  const enviar = async () => {
    ...
    try {
      ...
      setAberto(taskId as Id<"assistantTasks">);
      await executar({ taskId: taskId as Id<"assistantTasks"> });
    } catch (e) {
      toast.error(
  ...
  return (
    <div className="p-4 md:p-6 max-w-3xl mx-auto space-y-6">
      <header className="space-y-1">
        <h1 className="font-serif text-2xl md:text-3xl leading-tight">Pergunte ao ALTAR</h1>
        <p className="text-sm text-muted-foreground">
          Escreva o que você precisa. O ALTAR lê o que já está aqui — seus eventos, seu
          financeiro, seu funil — e responde. Ele analisa e organiza: não envia mensagem,
          não mexe em dinheiro e não apaga nada.
        </p>
      </header>
      ...
      <section className="space-y-3 rounded-xl border border-border bg-card p-4">
        ...
        <div className="flex flex-wrap items-center justify-end gap-2">
          ...
          >
            ...
            {enviando ? "Trabalhando…" : "Perguntar"}
          </Button>
        </div>

        {/* ── ESCOLHER QUEM CUIDA É OPCIONAL ─────────────────────────────
            Havia um seletor de sete agentes ao lado do botão, e ele fazia a
            pergunta errada: quem pergunta "quanto eu tenho a receber?" não
            deveria precisar saber que existe um Financeiro. O roteamento já é
            automático — o seletor só atrapalhava quem não tinha opinião.

            Fica como recolhido, e não removido, porque quem TEM opinião
            ("quero o olhar do Marketing sobre este evento") perde o pedido
            inteiro se o roteador discordar. */}
        <details className="group">
          <summary className="cursor-pointer list-none text-xs text-muted-foreground hover:text-foreground transition-colors">
            {agenteId
              ? `Vai cuidar: ${(equipe ?? []).find((a) => a.id === agenteId)?.nome ?? agenteId}`
              : "O ALTAR escolhe quem cuida · trocar"}
          </summary>
          <select
            value={agenteId}
            ...
            className="mt-2 h-11 w-full rounded-md border border-input bg-transparent px-3 text-base md:text-sm cursor-pointer"
          >
      ...
      {/* ── A EQUIPE ───────────────────────────────────────────────────────
          Cartões compactos, e agora recolhidos. Existem para ela saber quem
          existe e o que cada um alcança — nunca para escolher a cada pedido.
          Sete cartões abertos acima do histórico faziam a tela parecer um
          cardápio a ser decidido antes de perguntar qualquer coisa. */}

...
import { classificarPedido, recadoDaRecusa } from "./lib/assistente/semaforo";
import { rotear } from "./lib/assistente/roteamento";
import { AGENTES, ehAgenteId } from "./lib/assistente/agentes";
 ...
 * `assistenteExecutor.executar`, chamado pela tela em seguida — o mesmo
...
export const delegar = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (veredicto.cor === "vermelho") {
      return ctx.db.insert("assistantTasks", {
        userId: user._id,
        pedido,
...
/** A equipe. Constante, não cadastro — ver `lib/assistente/agentes.ts`. */
...
export const falhar = internalMutation({
  args: { taskId: v.id("assistantTasks"), erro: v.string() },
  handler: async (ctx, args) => {
    const tarefa = await ctx.db.get(args.taskId);
    if (!tarefa) return;
